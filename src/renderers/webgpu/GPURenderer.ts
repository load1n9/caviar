...
import type { DwmWindow } from "@gfx/dwm";
...
export class GPURenderer {
  ...
  constructor(public canvas: DwmWindow, public world: World) {
  }
  async init() {
    const adapter = await navigator.gpu.requestAdapter();
    if (!adapter) throw new Error(`Could not request adapter!`);
    const device = await adapter.requestDevice();
    const surface = this.canvas.windowSurface();
    this.#context = surface.getContext("webgpu");
    const devicePixelRatio = 1;
    const _presentationSize = [
      this.canvas.size.width * devicePixelRatio,
      this.canvas.size.height * devicePixelRatio,
    ];
  ...
  #setupRectangle(entity: Rectangle): void {
    ...
    for (let i = 0; i < data.length; i += 2) {
      data[i] = (data[i] / this.canvas.size.width) * 2 - 1;
      data[i + 1] = (data[i + 1] / this.canvas.size.height) * -2 + 1;
    }
  ...
  #renderRectangle(entity: Rectangle, renderPass: GPURenderPassEncoder): void {
    ...
    const x = (entity.x / this.canvas.size.width) * 2;
    const y = (entity.y / this.canvas.size.height) * -2;
  ...
  //   for (let i = 0; i < data.length; i += 2) {
  //     data[i] = data[i] * entity.width / this.canvas.size.width * 2 - 1;
  //     data[i + 1] = data[i + 1] * entity.height / this.canvas.size.height * -2 + 1;
  //   }
  //   const position = createBuffer(this.#device!, new Float32Array(data));
  //   const tex2d = loadTexture(this.#device!, entity.image.rawData);
  //   const uniforms = new Uniforms2D(this.#device!, this.#layouts!.uniform);
